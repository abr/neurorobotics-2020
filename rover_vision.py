...
class RoverVision():
    ...
    def predict(self, images, targets=None, show_fig=False): #, save_folder='', save_name='prediction_results', num_pts=100):
        ...
        with nengo_dl.Simulator(self.net, minibatch_size=self.minibatch_size, seed=self.seed) as sim:
            ...
            if show_fig:
                fig = plt.Figure()
                # if not isinstance(targets, float):
                #     num_pts = min(len(targets), 100)
                num_pts = 100
                plt.plot(targets[-num_pts:], label='target', color='r')
                plt.plot(predictions[-num_pts:], label='predictions', color='k', linestyle='--')
                # else:
                # plt.plot(np.ones(100)*targets, label='target', color='r')
                # plt.plot(np.ones(100)*predictions, label='predictions', color='k', linestyle='--')
                plt.legend()
...
if __name__ == '__main__':
    ...
    #     predictions = rover.predict(images=training_images[ii], targets=training_targets[ii], show_fig=True)
