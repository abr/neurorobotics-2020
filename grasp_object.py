...
from utils import get_approach_path, osc6dof, osc3dof, second_order_path_planner, target_shift, first_order_arc
from utils import adapt as adapt_f
...
def run():
    ...
    adapt = adapt_f(in_index=in_index, spherical=spherical)
        ...
        while not end_sim:
            ...
            for reach in reach_list[reach_mode]:
                ...
                while not at_target:
                    ...
                    if sim_time > 5:
                        print('Time to simulate 5s: ', (timeit.default_timer() - start))
                        end_sim = True
                        break
                    sim_time += 0.001
                    ...
                    if reach_mode == 'reach_target':
                        error = np.linalg.norm(hand_xyz - (final_xyz + reach['offset']))
                        if error < 0.05:
                            pos = final_xyz + reach['offset']
                            vel = np.zeros(3)
                        else:
                            if not np.allclose(final_xyz, old_final_xyz, atol=1e-5):
                                traj_planner.reset(
                                    position=pos,
                                    target_pos=(final_xyz + reach['offset']))
