...
d2 = np.load('/home/tdewolf/Downloads/abr_analyze_driving_training_images_processed.npz')
...
plt.savefig('/home/tdewolf/Downloads/data/plots/targets.png')
plt.clf()
...
for ii, (d1_i, d2_i) in enumerate(zip(d1_indices, d2_indices)):
    ...
    if dist1 > 3.0 or dist2 > 3.0:  # arbitrary threshold
        ...
        a = plt.subplot(2, 1, 1)
        a.imshow(d1['images'][d1_i].reshape((res[0], res[1], 3)) / 255, origin='lower')
        plt.title('d1 target %i: %.3f, %.3f' % (ii, d1_targets[ii][0], d1_targets[ii][1]))

        b = plt.subplot(2, 1, 2)
        b.imshow(d2['images'][d2_i].reshape((res[0], res[1], 3)) / 255, origin='lower')
        plt.title('d2 target %i: %.3f, %.3f' % (ii, d2_targets[ii][0], d2_targets[ii][1]))

        plt.suptitle('dist = %.3f' % max(dist1, dist2))

        plt.savefig('/home/tdewolf/Downloads/data/plots/%i.png' % ii)
        plt.clf()
